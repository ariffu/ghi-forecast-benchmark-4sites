#!/usr/bin/env python3
"""
R8 Arm B v2 (LENGKAP, +DL) -- JAMBI

Kelanjutan dari train_ghi_1h_jambi_R8_armB_v2.py (sesi sebelumnya, GBM-only:
CatBoost R2=0.6928, LightGBM R2=0.6937, n_test=21.129). Sesi itu menandai DL
sebagai "not executed" karena PyTorch tak terpasang di SANDBOX-nya. Di
environment lokal ini (Python 3.9 di "C:\\Program Files\\Python39"), torch
2.8.0+cpu TERPASANG dan sudah terbukti jalan (lihat run Arm B v1 Jambi
sebelumnya) -- jadi kendala itu tidak berlaku di sini, dan Arm B v2 dilengkapi.

Data source (sama persis dgn armB_v2 GBM-only, DIVERIFIKASI):
    jambi_ghi_forecast_1h_train_3h_rollback_2021_2025.duckdb
    -> jambi_sch.ghi_forecast_1h_train_3h_rollback_2021_2025

Catatan: cache pickle sesi sebelumnya (arm_B_v2_data.pkl) dibangun dgn numpy>=2.x
(numpy._core) dan TIDAK bisa dimuat di numpy 1.26 environment ini -> data
dibangun ULANG dari SQL memakai fungsi IMPOR LANGSUNG dari
train_ghi_1h_jambi_R1_benchmark_v2.py (connect_data/build_sql/add_features),
sehingga logika 100% identik, cuma tidak lewat cache.

Fair-play (identik template Bengkulu commit 836a397 & armB v1 Jambi):
  - Fitur: F1 lean-50 (sama dgn R1 v2 / Arm A v2 / Arm C v2)
  - Split: train<2024, val 2024, test 2025
  - GBM: CatBoost iterations=4000 lr=0.02 d8 ES150; LightGBM n_estimators=6000 lr=0.02 ES150
  - DL: LSTM/MLP/Transformer, 3 seed, ES patience=30, scaler fit train-only, seq_len=1
  - Target: point t+60

Run:
    & "C:\\Program Files\\Python39\\python.exe" train_ghi_1h_jambi_R8_armB_v2_DL.py
"""
import sys
import time
...
from train_ghi_1h_jambi_R1_benchmark_v2 import (
    connect_data, build_sql, add_features,
    FEATURES as F1_FEATURES, TARGET_POINT, TIME_COL,
...
# ---------------------------------------------------------------------------
# DL Architectures -- identik template Bengkulu commit 836a397 (bug-fixed)
# ---------------------------------------------------------------------------
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        out1, _ = self.lstm1(x)          # pass OUTPUT sequence, bukan hidden state
...
class MLPModel(nn.Module):
    ...
    def forward(self, x):
        return self.net(x[:, -1])        # flat_size = n_features (seq_len=1)
...
def main():
    ...
    print(f"DB reference: {LOCAL_DB_PATH.resolve()}  (schema={SCHEMA}, table={TABLE})", flush=True)

    print("Rebuilding dataset from SQL (v2, 24h-penuh, cache numpy-incompatible)...", flush=True)
    con = connect_data()
    ...
    print(f"Rows: {len(df_use):,}  (t={time.time()-t0:.0f}s)", flush=True)
    ...
    print(f"\n{'='*60}\nGBM MODELS (verifikasi ulang vs run sesi sebelumnya)\n{'='*60}", flush=True)
    ...
    results.append(m)
    print(f"  CatBoost  R2={m['r2']:.4f}  MAE={m['mae']:.1f}  RMSE={m['rmse']:.1f}  "
          f"iter={cb.get_best_iteration()}  (referensi sebelumnya: 0.6928)", flush=True)
    ...
    print(f"  LightGBM  R2={m['r2']:.4f}  MAE={m['mae']:.1f}  RMSE={m['rmse']:.1f}  "
          f"(referensi sebelumnya: 0.6937)", flush=True)
    ...
    for model_name, ModelCls in dl_models:
        ...
        for seed in DL_SEEDS:
            ...
            print(f"    {model_name} seed {seed}: R2={m['r2']:.4f}  (t={time.time()-t0:.0f}s)", flush=True)
        mean_r2 = np.mean(seed_results)
        std_r2 = np.std(seed_results)
        print(f"  {model_name:<12s}  R2={mean_r2:.4f} +/- {std_r2:.4f}", flush=True)

    df_res = pd.DataFrame(results)
    df_res.to_csv(OUTPUT_DIR / "arm_B_results_v2_full.csv", index=False)

    summary_rows = []
    for mname, grp in df_res.groupby("model"):
        summary_rows.append({
            "model": mname, "type": grp["type"].iloc[0],
    ...
    df_sum.to_csv(OUTPUT_DIR / "arm_B_summary_v2_full.csv", index=False)

    print(f"\n{'='*60}\nSUMMARY -- R8 Arm B v2 JAMBI (point t+60, test 2025, n={te_m.sum():,})\n{'='*60}", flush=True)
    ...
    print(f"\n  -> outputs: {OUTPUT_DIR}/arm_B_results_v2_full.csv, arm_B_summary_v2_full.csv", flush=True)
