#!/usr/bin/env python3
"""
V10 Bengkulu GHI 1-hour-ahead forecasting - BEST RESULT, v8's 40 lean features
plus 3 acceleration (2nd-difference) features ported from Training_Banten.

This is the best test-set result found across the whole project so far
(R2=0.8212, beating v6's full 5-model stack of 0.8210 with FAR less complexity:
43 features and a single LightGBM model, vs v6's 143 features + bagging +
Ridge stacking).

Provenance - ported from Documents/Balai Obsidian/Balai/Training_Banten (notes
08-09), which found in its OWN ablation that acceleration (2nd diff of GHI/kT/
cloud optical thickness) contributes standalone signal distinct from the 1st-
difference ("delta"/"trend") features already in v6/v8. Two other Banten levers
were tested here too and did NOT transfer to Bengkulu (see
train_ghi_1h_bengkulu_v9_banten_techniques.py for the full experiment + why):
    - "Smart persistence" (kt_now x clearsky-at-target) ranked #2 by raw
      importance but a clean ablation showed it is REDUNDANT (R2 unchanged/
      slightly worse when added alone) - it duplicates info already in
      kt_now + clear_sky_ghi_now + ghi_now. High importance != causal value.
    - Cross-family ensemble diversity (LightGBM+CatBoost+XGBoost+MLP, Banten's
      other +0.0024 lever) made things WORSE here: Bengkulu's MLP only reached
      R2=0.76 (vs trees' 0.82) - far weaker than Banten's MLP (0.862 vs trees'
      0.87, a tiny gap). Averaging in a much-weaker member drags the ensemble
      below the best single model. Diversity only helps when members are
      comparably skilled, which was true in Banten but not here.
Net lesson: only the acceleration features generalized; smart-persistence and
multi-family ensembling did not, both for explainable, data-grounded reasons
above rather than blind transfer.
...
Run:
    python train_ghi_1h_bengkulu_v10_accel_lean.py

Outputs:
    outputs_v10_accel_lean/ghi_1h_v10_metrics.csv
    outputs_v10_accel_lean/ghi_1h_v10_predictions_test.csv
    outputs_v10_accel_lean/ghi_1h_v10_feature_importance.csv
    outputs_v10_accel_lean/ghi_1h_v10_diagnostics.png
    outputs_v10_accel_lean/models/*.joblib
"""
...
OUTPUT_DIR = Path("outputs_v10_accel_lean")
...
def main():
    ...
    metrics_path = OUTPUT_DIR / "ghi_1h_v10_metrics.csv"
    ...
    for key, val in pred_dict_test.items():
        pred_out[key] = val
    pred_out.to_csv(OUTPUT_DIR / "ghi_1h_v10_predictions_test.csv", index=False)
    ...
    imp.to_csv(OUTPUT_DIR / "ghi_1h_v10_feature_importance.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "ghi_1h_v10_diagnostics.png", dpi=160)
    plt.close()
