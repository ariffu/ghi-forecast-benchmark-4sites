#!/usr/bin/env python3
"""
V6 Bengkulu GHI 1-hour-ahead forecasting - single horizon, stacked ensemble.

Builds on the best-performing prior baseline (v3/v4.1: LightGBM direct/residual on
ghi_forecast_1h_train_3h_rollback_2021_2025, train=2021-2023, valid=2024, test=2025,
test R2 ~ 0.815-0.819) and adds:
    1. SYNOP cloud-layer features (oktas, cloud layers, present/past weather) joined
       from synop_bengkulu_quality_final, same mapping used in v5.2.
    2. Finer-grained cloud/ghi lag features (10/20/30m) to capture faster cloud motion
       than the existing 60/180m lags.
    3. Clearsky-index (kt) engineered features.
    4. LightGBM with early stopping against the validation set instead of a fixed
       n_estimators, for both direct and residual targets, plus a seed-bagged direct
       model (variance reduction).
    5. A Ridge stacking meta-model fit on validation-set base predictions (no leakage),
       evaluated on the 2025 test set.

Deliverable is a single horizon (t+60m) pipeline - no multi-horizon, no segmentation.
...
Run:
    python train_ghi_1h_bengkulu_v6_stacked.py

Outputs:
    outputs_v6_stacked/ghi_1h_v6_metrics.csv
    outputs_v6_stacked/ghi_1h_v6_predictions_test.csv
    outputs_v6_stacked/ghi_1h_v6_feature_importance.csv
    outputs_v6_stacked/ghi_1h_v6_diagnostics.png
    outputs_v6_stacked/models/*.joblib
"""
...
OUTPUT_DIR = Path("outputs_v6_stacked")
...
def main():
    ...
    metrics_path = OUTPUT_DIR / "ghi_1h_v6_metrics.csv"
    ...
    for key, val in pred_dict_test.items():
        pred_out[key] = val
    pred_out.to_csv(OUTPUT_DIR / "ghi_1h_v6_predictions_test.csv", index=False)
    ...
    imp.to_csv(OUTPUT_DIR / "ghi_1h_v6_feature_importance.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "ghi_1h_v6_diagnostics.png", dpi=160)
    plt.close()
