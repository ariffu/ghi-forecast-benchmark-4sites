#!/usr/bin/env python3
"""
Focused 60-minute-ahead GHI residual benchmark for Bengkulu, patched for missing sequence rows.
...
OUTPUT_DIR = Path("outputs_60m_residual_models_v2")
...
def evaluate(pred_df, pred_cols):
    rows = []
    base_mask = np.isfinite(pred_df[TARGET_COL].values) & np.isfinite(pred_df["persistence"].values)
    persistence_rmse = float(np.sqrt(mean_squared_error(pred_df.loc[base_mask, TARGET_COL], pred_df.loc[base_mask, "persistence"])))
    for col in pred_cols:
        mask = base_mask & np.isfinite(pred_df[col].values)
        if mask.sum() == 0:
            continue
        rows.append(metric_row(pred_df.loc[mask, TARGET_COL].values, pred_df.loc[mask, col].values, col, persistence_rmse))
...
def segment_metrics(pred_df, best_col):
    ...
    for seg_col in ["target_ghi_bin", "hour", "solar_segment", "month", "has_clp"]:
        for val, group in pred_df.groupby(seg_col, dropna=False):
            if len(group) < 30:
                continue
            base_mask = np.isfinite(group[TARGET_COL].values) & np.isfinite(group["persistence"].values)
            if base_mask.sum() < 30:
                continue
            persistence_rmse = float(np.sqrt(mean_squared_error(group.loc[base_mask, TARGET_COL], group.loc[base_mask, "persistence"])))
            for col in ["persistence", "lgbm_residual", best_col]:
                if col not in group.columns:
                    continue
                mask = base_mask & np.isfinite(group[col].values)
                if mask.sum() < 30:
                    continue
                rows.append(metric_row(group.loc[mask, TARGET_COL].values, group.loc[mask, col].values, col, persistence_rmse) | {"segment_col": seg_col, "segment_val": str(val)})
    return pd.DataFrame(rows)
...
def main():
    ...
    # Sequence models only exist where a continuous 3-hour sequence is available. For operational ensemble columns,
    # fall back to LightGBM residual when sequence predictions are missing so metrics can use all test rows.
    attn_for_ensemble = test_df["attention_lstm_residual"].fillna(test_df["lgbm_residual"])
    trans_for_ensemble = test_df["transformer_residual"].fillna(test_df["lgbm_residual"])
    lstm_for_ensemble = test_df["lstm_residual"].fillna(test_df["lgbm_residual"])
    test_df["ensemble_lgbm_attention"] = clip_ghi(0.5 * test_df["lgbm_residual"].values + 0.5 * attn_for_ensemble.values)
    test_df["ensemble_lgbm_transformer"] = clip_ghi(0.5 * test_df["lgbm_residual"].values + 0.5 * trans_for_ensemble.values)
    test_df["ensemble_all_residual"] = clip_ghi((test_df["lgbm_residual"].values + attn_for_ensemble.values + trans_for_ensemble.values) / 3.0)
    test_df["ensemble_weighted_high"] = clip_ghi(0.4 * test_df["lgbm_weighted_residual"].values + 0.3 * attn_for_ensemble.values + 0.3 * trans_for_ensemble.values)
    test_df["ensemble_lgbm_lstm_transformer"] = clip_ghi((test_df["lgbm_residual"].values + lstm_for_ensemble.values + trans_for_ensemble.values) / 3.0)

    pred_cols = [c for c in test_df.columns if c in ["persistence", "lgbm_residual", "lgbm_weighted_residual", "lstm_residual", "attention_lstm_residual", "transformer_residual", "ensemble_lgbm_attention", "ensemble_lgbm_transformer", "ensemble_all_residual", "ensemble_weighted_high", "ensemble_lgbm_lstm_transformer"]]
