#!/usr/bin/env python3
"""
R8 Arm B — Kalbar: GBM vs Deep Learning Fair-Play Comparison
...
Bug fixes vs previous comprehensive script:
  ...
  - Uses Kalbar DB and column names, not Bengkulu

Run:
    & "C:\\Program Files\\Python39\\python.exe" train_ghi_1h_kalbar_R8_armB.py
"""
...
def add_features(df):
    ...
    # Rename columns to standard names (Kalbar specific mappings)
    ...
    out["ghi_lag_120m"] = out["ghi_now"].shift(12)   # 12 × 10min = 120min
    out["ghi_lag_180m"] = out["ghi_now"].shift(18)   # 18 × 10min = 180min
    ...
    out["clp_cot_lag_20m"] = out["clot_lag10m"] * 0.67 + out["clp_cot"] * 0.33  # interpolate
    out["clp_cot_lag_30m"] = out["clot_lag30m"]
    out["clp_cot_lag_60m"] = out["clp_cot"].shift(6)  # 6 × 10min = 60min
    ...
    out["smart_persist_avg"] = out["kt_now"] * out["ghi_cs_t60"]  # Kalbar may not have ghi_cs_avg_t10_t60
...
def main():
    ...
    missing = [f for f in F1_FEATURES if f not in df_use.columns]
    if missing:
    ...
    print(f"  GBM 2nd:  LightGBM R²={lgb_r2:.4f}  (delta vs CB: {lgb_r2-cb_r2:+.4f})")
    ...
    for mname in ["lstm", "mlp", "transformer"]:
        ...
        print(f"  DL {mname:<12s} R²={mean:.4f} ± {std:.4f}  (delta vs CatBoost: {delta_vs_cb:+.4f})")
